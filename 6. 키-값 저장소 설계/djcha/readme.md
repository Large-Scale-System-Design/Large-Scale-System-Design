...
그래서, 이걸로 장애 처리를 어떻게 이용해먹느냐?

두 서버간에 데이터를 Merkle Root 로 데이터가 같은지 확인하고 / 해시 데이터가 다른부분만 하위 hash(지문이라고도 표현)로 내려가면서 어떤 데이터가 다른건지 확인
...
<br>
            ...
            - 데이터 센터 장애 처리(데이터 센터 분산) → ~~카카오 화재 ㅋ (어차피 대기업도 안하는데 우리도 해야되나 의문 ^\_^;;;)~~
...
> 머클 트리, 블룸 필터, 데이터 동기화, heartbeat  
> <br>
> heartbeat: N대의 서버에서 특정 서버가 장애난걸 판단하는 처리  
